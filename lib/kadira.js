...
Kadira.connect = function(appId, appSecret, options) {
  ...
  options.newRelicId = options.newRelicId;
  options.payloadTimeout = options.payloadTimeout || 1000 * 20;
  options.endpoint = options.endpoint || "https://kadira.riffyn.com";
  ...
  } else if (options.newRelicId) {
    options.newRelicId = options.newRelicId.trim();

    Kadira.newRelicReporter = new NewRelicReporter(options.newRelicId);

    Kadira.coreApi = new KadiraCore({
      appId: options.newRelicId,
      appSecret: "",
      endpoint: options.endpoint,
      hostname: options.hostname
    });

    logger('Kadira reporting metrics to NewRelic: ', options.newRelicId)
    console.log('Kadira: scheduling metrics reporting to NewRelic');
    Kadira._schedulePayloadSend();

  } else {
    throw new Error('Kadira: required appId and appSecret or newRelicId');
  }
...
Kadira._sendPayload = function (callback) {
  new Fibers(function() {
    ...
    console.log('ramr: kadira sending payload:' + JSON.stringify(payload));
    ...
    if (Kadira.newRelicReporter &&
        (Kadira.newRelicReporter.report instanceof Function) ) {
      console.log('Kadira: Reporting metrics via New Relic ...');
      Kadira.newRelicReporter.report(payload);
      callback();
    }
